...
import { fetchDaily } from '../api/ccusage';
...
export default function DailyReport() {
  ...
  const { data, loading, error, refresh } = useCcusageData(
    () => fetchDaily({ ...params, agent }),
    [agent, params.since, params.until, params.offline]
  );

  if (error) {
    return <Result status="error" title="日报数据加载失败" subTitle={error} />;
  }
  ...
  return (
    <Spin spinning={loading}>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col span={24}>
          <UsageTable data={daily} periodLabel="日期" loading={loading} />
        </Col>
      </Row>
    </Spin>
  );
