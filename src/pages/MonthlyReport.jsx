...
import { fetchMonthly } from '../api/ccusage';
...
export default function MonthlyReport() {
  ...
  const { data, loading, error, refresh } = useCcusageData(
    () => fetchMonthly({ ...params, agent }),
    [agent, params.since, params.until, params.offline]
  );

  if (error) {
    return <Result status="error" title="月报数据加载失败" subTitle={error} />;
  }
  ...
  return (
    <Spin spinning={loading}>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} lg={14}>
          <TokenTrendChart data={monthly} />
        </Col>
        <Col xs={24} lg={10}>
          <CostTrendChart data={monthly} />
        </Col>
        <Col span={24}>
          <UsageTable data={monthly} periodLabel="月份" loading={loading} />
        </Col>
      </Row>
    </Spin>
  );
